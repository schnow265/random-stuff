...
x, y = 120, 160
Modus = ["Erscheinen", "Bewegen", "Drehen", "Verschwinden"]
...
for Nr in range(0,4) :
  ...
  Knopf[Nr].place(x=30+Nr*135, y=Hoehe-50, width=130, height=30)
Knopf[0].config(command=lambda : Spieler.showImage(x,y,1))
Knopf[1].config(command=lambda : Spieler.moveImage(20,Breite-200))
Knopf[2].config(command=Spieler.turnImage)
Knopf[3].config(command=Spieler.hideImage)
Fenster.mainloop()
