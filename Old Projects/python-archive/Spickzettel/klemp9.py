...
def initMenu() :
  ...
  #--
  Dateimenu.add_command(label="öffnen", command=openFile, accelerator="Strg+O")
  Dateimenu.add_command(label="Speichern", command=saveFile, accelerator="Strg+S")
  Dateimenu.add_command(label="Ende", command=closeAll, accelerator="Strg+X")
...
def saveFile() :
  ...
  if Name != "" :
    saveDiagnose(Name)
def closeAll() :
  if messagebox.askyesno("Ende", "Daten speichern?") :
    saveDiagnose("backup.txt") 
  Fenster.destroy()
def showInfo() :
  messagebox.showinfo("Info", "Der Klempner für deine Seele")
...
Fenster.bind('<Control-o>', getShotcut)
Fenster.bind('<Control-s>', getShotcut)
Fenster.bind('<Control-x>', getShotcut)
...
# Schluss-Ereignis
Fenster.protocol("WM_DELETE_WINDOW", closeAll)
